...
func (lbc *loadBalancerController) getUpstreamServers(ngxCfg config.Configuration, data []interface{}) ([]*ingress.Upstream, []*ingress.Server) {
	...
	for _, ingIf := range data {
		...
		for _, rule := range ing.Spec.Rules {
			...
			//hostnametype := strings.Split(host, "-")
			//if len(hostnametype) == 2 {

			//	continue //skip custom
			//}

			server := servers[host]
			if server.ConfigType == "Custom" && server.HasCustomConfig {
				continue //skip custom
			}
			if server == nil {
...
type customServer []string

func (s customServer) isCustomServer(server string) bool {
...
func (s customServer) getCustomServers(data []interface{}) []string {
	for _, ingIf := range data {
		ing := ingIf.(*extensions.Ingress)
		customServers, err := configtype.ParseAnnotations(ing)
		if err != nil {
			glog.V(3).Infof("error reading configtype in Ingress %v/%v: %v", ing.GetNamespace(), ing.GetName(), err)
			return nil
		}
...
func (lbc *loadBalancerController) createServers(data []interface{}, upstreams map[string]*ingress.Upstream) map[string]*ingress.Server {
	...
	var customServers customServer = customServer{}.getCustomServers(data)
	...
	for _, ingIf := range data {
		...
		for _, rule := range ing.Spec.Rules {
			...
			//hostnametype := strings.Split(host, "-")
			//if len(hostnametype) == 2 {
			//	//glog.Infof("get custom ingress config!", "host:", hostnametype[0], "type", hostnametype[1]) //debug
			//	host = hostnametype[0]
			//	if _, ok := servers[host]; ok {
			//		glog.V(3).Infof("rule %v/%v uses a host already defined. Skipping server creation", ing.GetNamespace(), ing.GetName())
			//	} else {
			//		configNsName := strings.Split(rule.HTTP.Paths[0].Path, "/")
			//		configNs := configNsName[1]
			//		configName := configNsName[2]
			//		configMap, err := lbc.getConfigMap(configNs, configName)
			//		if err != nil {
			//			glog.Errorf("get custom configmap", configNs, configName, "error")
			//			continue
			//		}
			//		nginxConfig := configMap.Data["custom"]

			//		servers[host] = &ingress.Server{Name: host, ConfigType: "Custom", CustomConfig: nginxConfig}

			//	}
			//	continue
			//}
			if customServers.isCustomServer(host) {
				if _, ok := servers[host]; ok {
					glog.V(3).Infof("rule %v/%v uses a host already defined. Skipping server creation", ing.GetNamespace(), ing.GetName())
				} else {
					configNsName := strings.Split(rule.HTTP.Paths[0].Path, "/")
					...
					if hasCustomConfig {
						servers[host] = &ingress.Server{Name: host, ConfigType: "Custom", CustomConfig: nginxConfig, HasCustomConfig: hasCustomConfig}
						continue
					}
				}
			}
